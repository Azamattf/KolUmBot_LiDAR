...
def plot_histogram(label_data):
    """
    Args:
        label_data (dict): Nested dictionary {camera_id: {label_name: count}}.
    """
    ...
    # Width of each bar
    bar_width = 0.2  # Smaller width for grouped bars
    num_cameras = len(label_data)
    offset = 0  # Offset for grouped bars

    # Plot bars for each camera
    for i, (camera_id, label_counts) in enumerate(label_data.items()):
        counts = [label_counts.get(label, 0) for label in all_labels]
        ax.bar(x + offset, counts, width=bar_width, label=camera_id)
        offset += bar_width  # Offset for the next group

    # Set x-axis labels and ticks
    ax.set_xticks(x + bar_width * (num_cameras - 1) / 2)  # Center labels
    ax.set_xticklabels(all_labels, rotation=45, ha="right")
    ax.set_xlabel("Labels")
    ax.set_ylabel("Counts")
    ax.set_title("Label Distribution by Camera")
